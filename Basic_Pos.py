...
def CostOfOrder():
    ...
    BurgerCost ="Php.",str('%.2f' % (BurgerPrice * 70.00))
    CostofBurgers.set(BurgerCost)
    
    SodaCost = "Php.", str('%.2f' % (SodaPrice * 40.00))
    CostofSoda.set(SodaCost)
    
    FriesCost = "Php.", str('%.2f' % (FriesPrice * 50.00))
    CostofFries.set(FriesCost)
    
    DeliveryCharges =  "Php." , str('%.2f' % (DeliveryCost * 30.00))
    CostofDelivery.set(DeliveryCharges)
    
    ToC= "Php.", str('%.2f'% ((BurgerPrice * 70.00) + (SodaPrice * 40.00)+ (FriesPrice * 50)
                                + (DeliveryCost * 30)))
    SubTotal.set(ToC)
    
    gst = (0.025+0.025)
    Tax= "Php" , str('%.2f'% (((BurgerPrice * 70.00)+(SodaPrice * 40.00)+(FriesPrice*50)+(DeliveryCost * 30))*gst))
    PaidTax.set(Tax)
    ...
    CostOfItems = "Php." , str('%.2f'%(TaxPay + Cost))
